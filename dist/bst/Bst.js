...
import { COMPARISON_RESULT } from './utils/comparator';
...
export class BST {
    ...
    _insertByRecursion(parentNode, newNode) {
    ...
    _findNodeByRecursion(parentNode, value) {
    ...
    findLargestNodeByIteration(node) {
        if (node.isLeaf()) {
            return node;
        }
        let parentNode = node;
    ...
    _traverseInOrderByRecursion(parentNode, result) {
        const leftNode = parentNode.getLeft();
        if (leftNode) {
    ...
    _traversePreOrderByRecursion(parentNode, result) {
        result.push(parentNode.getValue());
        const leftNode = parentNode.getLeft();
        if (leftNode) {
    ...
    _traversePostOrderByRecursion(parentNode, result) {
        const leftNode = parentNode.getLeft();
        if (leftNode) {
    ...
    printTreeByIteration(rootValue, printFn) {
        if (this.getSize() === 0) {
    ...
    /* istanbul ignore next */
    _traverseForPrint(parentNode, depth, direction, logs) {
        const SPACE = '    ';
        const rightNode = parentNode.getRight();
        if (rightNode) {
        ...
        let log = `${spaceString}${directionString} ${JSON.stringify(parentNode.getValue())}${parentNode.getExistCount() > 1 ? `(${parentNode.getExistCount()})` : ''}`;
        logs.push(log);
        const leftNode = parentNode.getLeft();
        if (leftNode) {
